#!/usr/bin/env sh
# Patched https://github.com/daeuniverse/dae-installer for openwrt
...
check_local_version() {
    if ! command -v /usr/bin/dae >/dev/null 2>&1; then
        ...
        current_version=$(/usr/bin/dae --version | awk 'NR==1' | awk '{print $3}')
    fi
}
...
download_geoip() {
    ...
    if ! curl -L "$geoip_url" -o "$geoip_temp_dir"/geoip.dat; then
...
update_geoip() {
    check_share_dir
    cp "$geoip_temp_dir"/geoip.dat /usr/share/dae/
    rm -rf "$geoip_temp_dir"
...
download_geosite() {
    ...
    if ! curl -L "$geosite_url" -o "$geosite_temp_dir"/geosite.dat; then
...
update_geosite() {
    check_share_dir
    cp "$geosite_temp_dir"/geosite.dat /usr/share/dae/
    rm -rf "$geosite_temp_dir"
...
install_dae() {
    ...
    cp "$temp_dir""/dae-linux-""$MACHINE" /usr/bin/dae
    chmod +x /usr/bin/dae
    rm -f dae-linux-"$MACHINE".zip
...
download_example_config() {
    ...
    if ! curl -L "$example_config_url" -o /etc/dae/example.dae; then
        notify_example="yes"
    fi
}
...
download_bash_completion() {
    ...
    if ! curl -L "$bash_completion_url" -o /usr/share/bash-completion/completions/dae; then
...
download_zsh_completion() {
    ...
    if ! curl -L "$zsh_completion_url" -o /usr/share/zsh/site-functions/_dae; then
...
download_fish_completion() {
    ...
    if ! curl -L "$fish_completion_url" -o /usr/share/fish/vendor_completions.d/dae.fish; then
...
notify_configuration() {
    ...
    if command -v service >/dev/null 2>&1; then
        ...
        echo "service dae start"
        echo "${GREEN}You can enable dae service so it can be started at system boot:${RESET}"
        echo "service dae enable"
    else
        echo "${YELLOW}No service installed beacuse of missing procd,
    ...
    echo "chmod 600 /usr/etc/dae/config.dae"
    echo '----------------------------------------------------------------------'
    echo '----------------------------------------------------------------------'
}
